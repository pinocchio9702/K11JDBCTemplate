...
- Object queryForObject(String sql, RowMapper rowMapper)
	: 하나의 레코드를 반환하는 select계열의 쿼리문 실행시 사용된다.(Object 타입)
- int queryForInt(String sql, Object[] args)
	: 인파리미터가 있고, 쿼리문의 실행결과가 숫자를 반환하는 select계열의 쿼리문에
	사용된다.
- int update(String sql)
	: 인파라미터가 없는 update/delete/insert 쿼리문을 처리할때 사용
- int update(String sql, Object[] args)
	: 인파리미터가 있는 update/delete/insert 쿼리문을 처리할때 사용
 */
public class JDBCTemplateDAO {
	...
	// 생성자
	/*
	 * 컨트롤러에서 @Autowired를 통해 자동주입을 받았던 빈을 정적변수인 JdbcTemplateConst.template에 값을
	 * 할당하였으므로, DB연결정보를 DAO에서 바로 사용하기 위해 값을 가져온다.
	 */
	public JDBCTemplateDAO() {
		this.template = JdbcTemplateConst.template;
		System.out.println("JDBCTemplateDAO생성자 호출");
	}

	public void close() {
		// JDTCTEMPLATE에서는 사용하지 않음
		/*
		 * Spring 설정파일에서 빈을 생성하므로 자원을 해제하면 다시 new를 통해 자원해지를
		 */
	}
	...
	public void write(final SpringBbsDTO springBbsDTO) {
		/*
		 * 매개변수로 전다로디는 값을 익명 클래스내에서 사용할 때는 반드시 final로 선언하여 값의 변경이 불가능하게 해줘야 한다. final로
		 * 선언하지 않을경우 에러가 발생한다.
		 */
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				/*
				 * 답변형 게시판에서 원본글인 경우 idx와 bgroup은 반드시 일치해야 한다. 리스트에서 게시물 정렬시 bgroup을 통해 order
				 * by절을 구성하기 때문이다. 또한 nextval은 한문장에서 여러번 사용해도 같은 시퀀스를 반환해야한다.
				 */
	...
	public SpringBbsDTO view(String idx) {
		...
		/*
		 * queryForObject()는 반환결과가 0개이서나 2개이상인경우 예외가 발생되므로 반드시 예외처리를 해주는 것이 좋다.
		 */
		try {
			// 결과 값이 하나가 아니고 0개 이면 에러가 나므로 try로 예외처리 해줘야함
	...
	public void reply(final SpringBbsDTO dto) {
		// 답변쓰기 전 레코드 업데이트
		/*
		답변글을 입력하기 전 일괄 업데이트 처리
		 */
		replyPrevUpdate(dto.getBgroup(), dto.getBstep());

		// write와 다른 점은 bgroup에 기존게시물의 번호가 들어가는 것이다.
		String sql = " INSERT INTO springboard  " + "  (idx, name, title, contents, pass, "
				+ "  bgroup, bstep, bindent)  " + "  VALUES  " + "  (springboard_seq.nextval, ?, ?, ?, ?,"
				+ "  ?, ?, ?)";
